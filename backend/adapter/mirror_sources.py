...
def fetch_5paisa(symbol):
    ...
    # Contract selector begins immediately after the product category on the rendered page.
    ...
    # First rupee price after the active-expiry text is the displayed MCX contract price.
    tail=txt[m.end():]
...
def fetch_icici_active():
    ...
    if not rows:
        # bounded text fallback for server-rendered pages whose table tags change
        txt=_text(raw)
...
def _normalized(symbol,q,now):
    ...
    return {"commodity":display,"source_symbol":symbol,"instrument":f"{display.title()} Futures ({q.expiry.strftime('%d%b%Y').upper()})","ltp":q.ltp,"breakout_level":r1,"breakdown_level":s1,"buy_invalidation_below":p,"sell_invalidation_above":p,"atr":None,"trend_bias":bias,"source_timestamp":q.trade_time.isoformat(),"source_trade_date":q.trade_time.date().isoformat(),"verified":True,"derivation":"cross-source verified 5paisa OHLC + ICICI exact-contract verification; classic pivot P/R1/S1","verification_tier":"CROSS_SOURCE_VERIFIED"}

def acquire_mirrors(now=None):
    now=now or datetime.now(timezone.utc); local=now.astimezone(IST); diagnostics={"adapter":"mcx-multi-source-v1","mode":"mirror_reconciliation","fetched_at":now.isoformat(),"providers":["5paisa","ICICI Direct"],"verified_count":0,"commodities":{},"errors":[]}
    # This adapter is intentionally EOD/premarket only. Mid-session snapshots are not EOD data.
    if local.hour>=9:
    ...
    for symbol in TARGETS:
        d={"status":"NOT_RECOMMEND","sources":{},"checks":{}}
        try:
            q,qdiag=fetch_5paisa(symbol); d["sources"]["5paisa"]={"url":q.url,"expiry":q.expiry.isoformat(),"trade_time":q.trade_time.isoformat(),**qdiag}
            ...
            if age<0 or age>10080: raise ValueError(f"5paisa timestamp stale/future: {age:.1f} min")
            d["status"]="CROSS_SOURCE_VERIFIED"; d["contract"]=token; d["trade_date"]=q.trade_time.date().isoformat(); diagnostics["verified_count"]+=1; out.append(_normalized(symbol,q,now))
        except Exception as exc:
