...
def live(c,now,price,**kw):
    base={"commodity":c,"instrument":f"{c} Futures (31AUG2026)","price":price,"timestamp":(now-timedelta(minutes=1)).isoformat(),"verified":True}
    base.update(kw); return base


class EngineTests(unittest.TestCase):
    def setUp(self): self.now=datetime(2026,8,21,3,31,tzinfo=timezone.utc)
    ...
    def test_post_open_mode_missing_current_fails_closed(self):
        rows,audit=run([eod(c,self.now) for c in COMMODITIES],self.now,require_current=True)
        self.assertTrue(all(r["verdict"]=="NOT_RECOMMEND" for r in rows))
    ...
    def test_current_observation_must_be_verified_and_fresh(self):
        source=eod("GOLD",self.now,trend_bias="BUY")
        rows,_=run([source],self.now,current_observations=[live("GOLD",self.now,120,verified=False)],require_current=True); self.assertEqual(rows[0]["verdict"],"NOT_RECOMMEND")
    ...
    def test_signal_trend_conflict_fails_closed(self):
        ...
        rows,_=run([source],self.now,current_observations=[live("GOLD",self.now,120)],require_current=True)
        self.assertEqual(rows[0]["verdict"],"NOT_RECOMMEND")
